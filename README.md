...
- `watchface`: WFF v2 face with six editable complication slots, localized static labels and ambient mode.
- `marine-provider`: Wear OS companion that selects the nearest of 51 launch spots or lets the surfer pin one manually, downloads one small static JSON, caches it and supplies the complications, including an 18-hour tide curve.
...
Install both packages on Wear OS 5+ and open **Tideglass Data** to use GPS or choose one of the 51 spots manually. The face requests the six Tideglass providers automatically on first use; users can still replace them in the watch-face editor. Release bundles are still debug-signed; replace that signing configuration before Play Console upload.

The **Build Wear OS preview** GitHub Action verifies the live 51-spot endpoint, runs tests and lint, and publishes both APK and AAB packages as a downloadable 14-day artifact. These preview packages are debug-signed and are not the final Play Console signing setup.
